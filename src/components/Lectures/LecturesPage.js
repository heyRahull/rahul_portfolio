...
const cleanMarkdown = (content) => {
  ...
  cleaned = cleaned.replace(/!\[\]\(https:\/\/cdn\.hashnode\.com\/[\s\S]*?\)/g, "").trim();
  return cleaned;
};
...
const LecturesPage = () => {
  ...
  const currentCourseData = useMemo(() => courseDataRegistry[courseId] || frontendSystemDesignData, [courseId]);
  const allItems = useMemo(() => currentCourseData.flatMap((section) => (section.items || []).map((item) => ({ ...item, sectionTitle: section.sectionTitle }))), [currentCourseData]);

  const [selectedLectureId, setSelectedLectureId] = useState(allItems[0]?.id || "");
  const [externalContent, setExternalContent] = useState("");

  const selectedLecture = useMemo(() => allItems.find((item) => item.id === selectedLectureId) || allItems[0], [selectedLectureId, allItems]);
